	...
	<table border="2">
	  ...
	  <tbody>
	    ...
	    while($row = $res->fetch_assoc()){
				$id=$row["ID"];
	          echo "<tr><td>{$row['user_id']}</td><td>{$row['Name']}</td><td>{$row['academic_unit']}</td><td>{$row['item_desc']}</td><td>{$row['cost']}</td><td>{$row['how_it_will_use']}</td><td>{$row['justification']}</td><td>{$row['classes_support']}</td><td>{$row['no_of_students']}</td><td>{$row['date']}</td><td>{$row['d_chair_approval']}</td><td>{$row['rank']}</td><td>{$row['c_chair_review']}</td><td>{$row['c_chair_comment']}</td><td>{$row['dean_approval']}</td><td><a href='Update_Request_Form.php?id=$id'>Update Form</a></td><td><a href='Update_Status.php?id=$id'>Update Status</a></td><td><a href='View_Members_Review.php?id=$id'>Committee Members Review</a></td></tr>\n";
	        }      
	    ?>
	  </tbody>
	</table>
